" Vim color file
" 2016/6/11
" This color scheme is a tweaked version of summerfruit256 for the 
" application of white and black background
" [BEGIN ORIGINAL INFO]
...
let g:colors_name="asou_summerfruit256"
...
if has("gui_running") || &t_Co == 88 || &t_Co == 256
    ...
    " set the highlighting for the given group with cterm and gui attrs
    fun <SID>Xex(group, fg, bg, cterm_attr, gui_attr)
        if a:fg != ""
        ...
        if a:cterm_attr != ""
            exec "hi " . a:group . " cterm=" . a:cterm_attr
        endif
        if a:gui_attr != ""
            exec "hi " . a:group . " gui=" . a:gui_attr
        endif
    endfun

    " sets the highlighting for the given group
    fun <SID>X(group, fg, bg, attr)
        call <SID>Xex(a:group, a:fg, a:bg, a:attr, a:attr)
    endfun
    ...
    call <SID>X("Pmenu", "000000", "e6f0f7", "")
    call <SID>X("PmenuSel", "ffff00", "438ec3", "")
    call <SID>X("SignColumn", "", "", "")
    hi CursorLine cterm=none guibg=#e6f0f7
    call <SID>X("CursorLineNr", "ffff00", "438ec3", "bold")
    ...
    call <SID>X("Error", "d40000", "ffcccc", "reverse")
    call <SID>X("Identifier", "ff0086", "", "")
    call <SID>X("Label", "ff0086", "", "")

    " Spell
    hi SpellBad cterm=reverse gui=undercurl guisp=red
    hi SpellCap cterm=reverse gui=undercurl guisp=blue
