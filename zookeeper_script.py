...
call(["ln", "-s", "kafka_2.11-0.10.1.0", "kafka"])

# download kafka server.properties
call(["wget", KAFKA_SERVER_CONFIG_LOCATION])
call(["mv", KAFKA_SERVER_CONFIG, "./kafka/config/server.properties"])
...
call(["./kafka/bin/kafka-server-start.sh -daemon ./kafka/config/server.properties"])
